...
export default class CartController {
    ...
    getOne = async(req, res, next) => {
        ...
        try {
            ...
            if (cid.length != 24) {
                CustomError.createError({statusCode: 400, name: "Error al leer base de datos", cause: generateErrorInfo.getId(cid), message: "Cart ID not valid", code: 2});
                req.logger.error(`Cart ID invalido: ${cid} en la ruta ${req.url}`);
            }
    ...
    putProduct = async(req, res) => {
        ...
        try {
            ...
            if (!cart) {
                ...
                req.logger.error(`Cart ID no válido: ${cid} en ${req.url}`);
            }
            let exist = false;
    ...
    delete = async(req, res) => {
        ...
        try {
            ...
            if (!cart) {
                CustomError.createError({statusCode: 404, name: "Cart doesnt exist", cause: generateErrorInfo.idNotFound(), code: 2});
                req.logger.error(`Cart ID no es válido: ${cid} en ${req.url}`);
    ...
    deleteProduct = async(req, res) => {
        ...
        try {
            ...
            if (!productExist) {
                ...
                req.logger.error(`Product ID no es válido: ${productId} en el cart ${id}. Ruta ${req.url}`);
            } else {
                let cart = await cm.getOne(id);
        
                if (!cart) {
                    CustomError.createError({statusCode: 404, name: "Cart doesnt exist", cause: generateErrorInfo.idNotFound(), code: 2});
                    req.logger.error(`Cart ID no es válido: ${id} en ${req.url}`);
                } else {

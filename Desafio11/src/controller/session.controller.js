...
export default class SessionController {
    ...
    try {
      ...
      try {
        transport.sendMail({
          ...
          html: `
                <div style="background-color: black; color: green; display: flex; flex-direction: column; justify-content: center;  align-items: center;">
                    <h1>Reestablecer contraseña<a href="http://localhost:8080/recoverLanding/${token}">Click aqui</a></h1>
                </div>
                `,
        });
      } catch (error) {
        return res.send({ status: "error", message: "email inválido" });
      }
  ...
  postRecoverPassword = async (req, res, next) => {
    try {
      ...
      if (result.acknowledged)
        res.send({ status: "Ok", message: "Contraseña actualizada" });
    } catch (error) {
      next(error);
    }
  };
  ...
  postSwapUserClass = async (req, res, next) => {
    try {
      ...
      req.logger.debug("Consigue los datos del usuario");
